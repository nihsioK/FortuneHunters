...
# To prevent CSRF attacks, websites use CSRF tokens, 
# which are random, secret values generated by the server 
# and passed to the client. These tokens are typically 
# sent as part of forms. When the user submits the form, 
# the server checks the token to ensure it matches the one 
# it expected to receive, thus confirming that the request 
# came from the legitimate user and not a malicious site.
...
# in web browsers that controls how web applications hosted at 
# one origin (domain) can interact with resources located on different 
# origins. It's a part of the security model that restricts how scripts 
# on a website can make requests to other websites, 
# which is known as the Same-Origin Policy (SOP).
...
#In Django, particularly when working with Django REST Framework (DRF),
# a "static file" refers to any file that is not dynamically generated 
# and does not change with each request. Common examples include images, 
# CSS files, JavaScript files, and other assets used in web applications